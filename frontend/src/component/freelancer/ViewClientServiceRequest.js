...
const ViewClientServiceRequest = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        const response = await api.get('http://localhost:8989/api/service-requests/listrequest/' + user.id, {
          headers: {
  ...
  const handleSubmitProposal = async (values, { setSubmitting }) => {
    try {
      ...
      const response = await api.post(`/proposals/create/` + selectedCategoryId, values);
...
export default ViewClientServiceRequest;
